...
def get(path: str, **params) -> dict | list:
    """GET mot API:t. Throttlar, backar av vid 429, höjer TokenExpired vid 401."""
    ...
    for attempt in range(5):
        ...
        if resp.status_code == 429:
            ...
            print(f"  429 rate limit - väntar {retry}s (försök {attempt + 1}/5)")
            time.sleep(retry)
            continue
        resp.raise_for_status()
        return resp.json()
    raise RuntimeError(f"Gav upp efter upprepade 429 på {path}")
